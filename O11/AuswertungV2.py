...
SP = pd.read_csv('O11/MessdatenO11SP.csv', header=1)
PP = pd.read_csv('O11/MessdatenO11PP.csv', header=1)
BN = pd.read_csv('O11/BerechnungNormierung.csv')
...
plt.plot(x_ax, y_ax_PP, label=f"Fit zu parallel polarisierten Licht mit Fitparameter \n $\\alpha_{{B,\\text{{parallel}}}} = {B_value_PP:.6f} \\pm {B_error_PP:.6f}$", linewidth=2, color='cornflowerblue')
...
# Plot zeichnen
plt.xlabel('Eingangswinkel $\\alpha_E$ in Radiant',fontsize=fnt)
plt.ylabel('$\\sqrt{R}$ in 1',fontsize=fnt)
plt.legend(fontsize=fnt)
plt.title("Wurzel des Reflexionsvermögen von parallel und senkrecht polarisierten Licht in Abhängigkeit des Einfallwinkels",fontsize=fnt)

plt.xticks(fontsize=fnt)
plt.yticks(fontsize=fnt)

plt.savefig("O11\O11Auswertung.pdf", format='pdf', bbox_inches='tight', pad_inches=0.5)
