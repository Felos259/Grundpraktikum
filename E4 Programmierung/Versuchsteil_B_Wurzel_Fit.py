...
def fit_function(x, A):
    return np.sqrt((wid)**2+(A * 2*np.pi*x)**2) #A entspricht der Induktivität der Spule, x0 entspricht R_RL
...
kapazitaet = unp.uarray([107.4],[0.2+0.02*107.4])*10**(-9)
print(kapazitaet)
print("Die erwartete Resonanzfrequenz liegt bei ",1/(2*np.pi*unp.sqrt(kapazitaet*induktivitaet)),"Hz")
print("Die Induktivität der Spule beträgt",induktivitaet,"H")
...
x_inset = np.linspace(-0.18, 32.518, 1000)
y_inset = fit_function(x_inset, A_value)
...
ax_inset.set_xlim(-0.18, 32.518)
ax_inset.set_ylim(-0.18, 5.00018)
ax_inset.set_xticks([0, 30])
ax_inset.set_yticks([0, 5])
ax_inset.tick_params(labelsize=8)

plt.savefig("VersuchsteilBWurzel.pdf", format='pdf', bbox_inches='tight', pad_inches=0.5) 
