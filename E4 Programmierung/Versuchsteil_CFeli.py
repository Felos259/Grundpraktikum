...
RF = pd.read_csv('E4 Programmierung/Versuchsteil_C_Feli.csv', header=1)
RG = pd.read_csv('E4 Programmierung/Versuchsteil_C_phi.csv', header=1)

uf = unp.uarray(RF['Frequenz'], RF['df']) # Frequenz der eingestellten Wechselspannung 
...
# Reiner ohmscher Widerstand R!!!!!
uR = u.ufloat(10.16,0.2016)

# R_RL = Summe aus Omhschen Widerstand und Spulenwiderstand
widerstand = uR+u.ufloat(151.02,10*0.01+1.5102) #in Ohm
...
plt.savefig("E4 Programmierung/VersuchsteilC1.pdf", format='pdf', bbox_inches='tight', pad_inches=0.5) 
...
# Periode und Verschierbung zw. Phasen der Channel in s
# Unsicherheit der Periodendauern beträgt einen mini Skalenstrich - also 1/5 der Auflösung * 10**-3
uverschiebung = unp.uarray((RF['t_VerK'] * RF['t'] *10**-3 / 5.0),  (RF['t'] *10**-3 / 5.0))


# Phis berechnen - modulo pi*2, um doppelte Verschiebung zu beachten
phi = uverschiebung * uf *2.0 * np.pi
adjustPhi = []


# for i in range(0, len(phi), 1):
#     if 0.75 >= phi[i].n and phi[i].n >= 0.25:
#         adjustPhi.append(phi[i].n-0.5)
#     elif phi[i].n > 0.75:
#         adjustPhi.append(phi[i].n-1)
#     elif 0.25 > phi[i].n and phi[i].n >= 0:
#         adjustPhi.append(phi[i].n)
#     else:
#         adjustPhi.append(phi[i].n)

# correctedPhi = unp.uarray(adjustPhi, [value.s for value in phi]) * 2.0 * np.pi
...
# Daten spezifizieren
x_data = [value.n for value in  uf]
x_err = [value.s for value in uf]
y_data = [value.n for value in correctedPhi]
y_err = [value.s for value in correctedPhi]

for i in range(0, len(x_data),1):
    print(x_data[i], y_data[i])

# Plot der Messwerte V und p mit Errorbars 
ax.errorbar(x_data, y_data, xerr=x_err , yerr=y_err, label='$\\varphi$ in Abhängigkeit der Frequanz der Wechselspannung', color = 'lightblue', linestyle='None', marker='o', capsize=6)

# Fitfunktion definieren
def fit_function2(x, A, B): #A = L und B = C laut Theorie
    return (2*np.pi*A*x - 1/(2*np.pi*B*x))/wid*1000
#A entspricht der Induktivität der Spule L, x0 entspricht R_RL, B entspricht der Kapazität C


# Nulllinie ziehen, um Trend zu erkennen
plt.axhline(0.0, color='lightgrey', linewidth=0.8, linestyle='--') 
# Nulllinie ziehen, um Trend zu erkennen
plt.axhline(np.pi /2, color='lightgrey', linewidth=0.8, linestyle='-') 
plt.axhline(-np.pi /2, color='lightgrey', linewidth=0.8, linestyle='-') 
...
x_ax = np.linspace(0.0001, 1800, 10000) #Hier NICHT bei 0 anfangen, da man sonst einen Divide-by-zero-Fehler erzeugt!
...
plt.plot(x_ax, np.arctan(( 2*np.pi*x_ax * i_value -  1/(2*np.pi * x_ax * k_value))/wid)  ,label=f"Aus den Werten von $C$,$L$, $R_{{RL}}$ resultierender Fit", linewidth=1.5, color='orange')
plt.plot(x_ax, -np.pi + np.arctan(( 2*np.pi*x_ax * i_value -  1/(2*np.pi * x_ax * k_value))/wid)  ,label=f"Aus den Werten von $C$,$L$, $R_{{RL}}$ resultierender Fit", linewidth=1.5, color='orange')


#Teilt man die Zeile über dieser Zeile durch 1000, so kommt ein relativ okayer Fit heraus
plt.xlabel('Frequenz $f$ in $s^{-1}$')
plt.ylabel("Phasenverschiebung $\\varphi$")
plt.legend()
plt.title("$f$-$Tan(\\varphi)$-Diagramm")

plt.savefig("E4 Programmierung/VersuchsteilCPHIIIIS.pdf", format='pdf', bbox_inches='tight', pad_inches=0.5) 
